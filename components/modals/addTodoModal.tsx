...
const AddTodoModal = ({
      ...
      try {
        ...
        parsedResult.forEach(data => {
          ...
          highlightedElements[startIndex] = (
            <ThemedText key={`date-${startIndex}`} style={styles.highlight}>
              {parsedText}
            </ThemedText>
          );
        });
        ...
        text.split(' ').forEach((word: string) => {
          if (word.startsWith('@')) {
            ...
            highlightedElements[startIndex] = (
              <ThemedText key={`mention-${startIndex}`} style={styles.mention}>
                {word}
              </ThemedText>
            );
        ...
        .map(([labelKey, labelValue]) => {
          tooltipRows.push(
            <TouchableOpacity key={labelValue.id} onPress={() => handleLabelPress(labelValue.name)}>
              <ThemedText>{labelValue.name}</ThemedText>
            </TouchableOpacity>,
          );
      ...
      if (
        ...
        tooltipRows.push(
          <TouchableOpacity
            ...
            <ThemedText>+ create Label {selectedSection}</ThemedText>
          </TouchableOpacity>,
        );
      }
    }
  ...
  const updateTextIfSymbolIsLastChar = (symbol: string, option: string) => {
    ...
    const newText = (
      <ThemedText key={`${symbol}${option}`} style={styles.mention}>
        {symbol + option}
      </ThemedText>
    );
    ...
    <Modal
      ...
      <View style={styles.inputModal}>
        <TextInput
          ...
          blurOnSubmit={false}>
          <ThemedText style={styles.highlightContainer}>{highlightedText}</ThemedText>
        </TextInput>
        <TextInput
          placeholder="Summary"
          value={summary}
